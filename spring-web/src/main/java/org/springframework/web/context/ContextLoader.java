...
public class ContextLoader {
	...
	//在ContextLoader.java的路径下，有一个ContextLoader.properties文件，查找并打开这个文件
	static {
	 ...
	 * 继续进入initWebApplicationContext方法，这个方法在其父类ContextLoader实现，
	 * 根据方法名可以看出这个方法是用于初始化一个WebApplicationContext，简单理解就是初始化一个Web应用下的Spring容器
	 */
		...
		try {
			...
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					//这里关注重点代码configureAndRefreshWebApplicationContext(cwac,servletContext)，
					// configureAndRefreshWebApplicationContext方法具体代码如下
					configureAndRefreshWebApplicationContext(cwac, servletContext);
				}
			}
			//可以看到，这里将初始化后的context存到了servletContext中，具体的就是存到了一个Map变量中，
			// key值就是WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE这个常量
	 ...
	 * @see ConfigurableWebApplicationContext
	 * 这个方法主要用于创建一个WebApplicationContext对象。因为WebApplicationContext只是一个接口，不能创建对象，
	 * 所以需要找到一个WebApplicationContext接口的实现类，determineContextClass方法就是用于寻找实现类
	 */
